...
class PainterroProc {
  constructor(params) {
    ...
    this.tools
      ...
      .forEach((b) => {
        ...
        const btn =
          `<button type="button" class="ptro-icon-btn ptro-color-control" title="${tr(
            `tools.${b.name}`
          )}${hotkey}" ` +
          `id="${id}" >` +
    ...
    this.wrapper.innerHTML =
      ...
      `<div>THIS IS A TEST INNER GRID MAYBE GOES HERE</div> ${this.testFunction()}` +
  ...
  testFunction() {
    console.log("TEST");
    return (
      <div>hello world idk this is a test to see if i can return html</div>
    );
  }
